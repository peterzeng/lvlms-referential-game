...
METRICS = [
    ...
    ("re_words", "Number of RE Words\n(Lexical Entrainment)"),
    ("relative_lexical_overlap", "Proportion of Lexical Overlap\n(Lexical Entrainment)"),
]

CONDITION_ORDER = [
    ("ACL", "ACL_prompt", "gpt-5.2", "Implicit Prompt, GPT-5.2"),
    ("ACL", "ACL_prompt", "gpt-5.5", "Implicit Prompt, GPT-5.5"),
    ("Cameron", "cameron-prompt", "gpt-5.2", "Explicit Prompt, GPT-5.2"),
    ("Cameron", "cameron-prompt", "gpt-5.5", "Explicit Prompt, GPT-5.5"),
]

COLORS = {
    "Implicit Prompt, GPT-5.2": "tab:blue",
    "Implicit Prompt, GPT-5.5": "tab:orange",
    "Explicit Prompt, GPT-5.2": "tab:blue",
    "Explicit Prompt, GPT-5.5": "tab:orange",
}

MARKERS = {
    "Implicit Prompt, GPT-5.2": "o",
    "Implicit Prompt, GPT-5.5": "^",
    "Explicit Prompt, GPT-5.2": "o",
    "Explicit Prompt, GPT-5.5": "^",
}

LINESTYLES = {
    "Implicit Prompt, GPT-5.2": ":",
    "Implicit Prompt, GPT-5.5": ":",
    "Explicit Prompt, GPT-5.2": "-",
    "Explicit Prompt, GPT-5.5": "-",
}
...
def plot_figure_subset(
    summary: pd.DataFrame,
    output_path: Path,
    metrics_subset: list[tuple[str, str]],
    ...
    fig, axes = plt.subplots(1, len(metrics_subset), figsize=figsize, constrained_layout=True)
    conditions = [label for *_, label in CONDITION_ORDER]

    if len(metrics_subset) == 1:
        axes = [axes]

    for idx, (ax, (metric, ylabel)) in enumerate(zip(axes, metrics_subset)):
        ...
        # Set y-axis label to "Value" for the leftmost plot of the figure, empty for others
        if idx == 0:
            ax.set_ylabel("Value")
        else:
            ax.set_ylabel("")
    ...
    fig.legend(handles, labels, loc="lower center", ncol=4, frameon=False, bbox_to_anchor=(0.5, -0.15))
...
def main() -> None:
    ...
    # Save the first 3 plots separately
    part1_metrics = METRICS[:3]
    plot_figure_subset(
        summary,
        output_base.with_name(f"{output_base.name}_part1"),
        part1_metrics,
        figsize=(12.0, 3.8),
    )
    
    # Save the last 2 plots separately
    part2_metrics = METRICS[3:]
    plot_figure_subset(
        summary,
        output_base.with_name(f"{output_base.name}_part2"),
        part2_metrics,
        figsize=(8.0, 3.8),
    )
    
    print(f"Wrote {output_base.with_name(f'{output_base.name}_part1.png')}")
    print(f"Wrote {output_base.with_name(f'{output_base.name}_part1.pdf')}")
    print(f"Wrote {output_base.with_name(f'{output_base.name}_part2.png')}")
    print(f"Wrote {output_base.with_name(f'{output_base.name}_part2.pdf')}")
