...
def process_csv_file(input_csv, output_folder):
    ...
    for pair_key, pair_data in pairs.items():
        ...
        # Get group IDs from rounds (they may change per round)
        group_ids = [r.get('group_id', '') for r in rounds.values() if r.get('group_id')]
        group_id_display = ', '.join(sorted(set(group_ids))) if group_ids else 'N/A'
        ...
        header += f"Group IDs: {group_id_display}\n"
        ...
        # Use first group ID for filename
        first_group_id = group_ids[0] if group_ids else 'unknown'
        filename = f"pair_session{session_code}_group{first_group_id}{prolific_part}.txt"
